...
class GarminFitExport():
    ...
    def debug_tree(self, output_dir : str) -> None:
        ...
        """
        filename_debug = os.path.join(output_dir,self.file_id.get('type'),os.path.basename(self.__pathfile)+"_bak.txt")
                    ...
                    if('intensity' in elem.keys()):
                        if('exercise_category' in elem.keys()):
                            if(elem['duration_type'] == 'time'):
                                ...
                                fmd.write("|{}|**{}**|*{}*|{}|\n".format(elem['message_index']+1, elem['intensity'].capitalize(), elem['exercise_category'].capitalize(), elem['duration_value'], elem['duration_type']))
                        else:
                            if(elem['duration_type'] == 'time'):
