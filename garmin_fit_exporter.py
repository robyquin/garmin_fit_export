...
parser.add_argument('-s', '--source', required = True, help='file or directory source')
parser.add_argument('-d', '--destination', required = True, help='directory destination')
parser.add_argument('--debug', action='store_true', help='Export debug raw file')
...
if(not os.path.isdir(output)):
    logging.error("{} : Destination doesn't exist or isn't a directory".format(output))
    exit()
...
if(os.path.isfile(input)):
    ...
    if(args.debug):
        ...
        if(file_type in ('activity', 'location', 'course')):
            ...
            logging.warning("Tipo file: {} non riconosciuto per esportazione".format(file_type))
